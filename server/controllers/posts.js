...
export const likePost = async (req, res)=>{
    try{
        ...
        const updatedatedPost = await Post.findbyIdAndUpdate( //we have to update post once like button is hit
            id,
            {likes: post.likes},
            {new: true}
        );

        res.status(200).json(updatedatedPost); //we finally pass in updated version to the front-end
    } catch (err) {
        res.status(409).json({message: err.message})
    }
}